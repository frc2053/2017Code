...
void AlignCenter::Execute()
{
	...
	if(abs(Robot::oi->GetLeftXAxisDriver()) > .2 || abs(Robot::oi->GetLeftYAxisDriver()) > .2 || abs(Robot::oi->GetRightXAxisDriver()) > .2) {
		isDone = true;
	...
	Robot::drivebaseSubsystem->MecanumDrive(speedX, 0, 0, adjyaw);
}
