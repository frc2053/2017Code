...
RED_DriveToBoiler::RED_DriveToBoiler(std::string pos) {
	...
	if(!gearPos.compare("center")) {  //returns 0 if equal so thats why we are doing !
		...
		AddSequential(new PrintCommand("Starting RED Boiler From Center"));
	...
	if(!gearPos.compare("rightred")) { //if (left and blue) or (right and red)
	...
	if(!gearPos.compare("leftred")) { //if (right and blue) or (left and red)
